...
with tab_a14:
    ...
    bl = ["BW","BY","BE","BB","HB","HH","HE","MV","NI","NW","RP","SL","SN","ST","SH","TH","LU","AT"]
    logos = [f"app/static/logos/{x}.png" for x in bl]
    ...
    column_config = {
        "bl": st.column_config.ImageColumn("Bundesländer")
        }
    
    for key in list(df_c2_a14_w.keys()):
        column_config[key] = st.column_config.ProgressColumn(
            key,
            #help="help",
            format="%.4fGW",
            min_value=0,
            max_value=df_c2_a14_w[key].sum()
        )
    
    df_t = df_c2_a14_w.copy()
    df_t["bl"] = logos
    df_t = df_t.set_index("bl")
